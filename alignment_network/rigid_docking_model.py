...
class IEGMN(nn.Module):
    ...
    def forward(self, batch_hetero_graph, epoch):
        ...
        orig_coors_receptor = batch_hetero_graph.nodes['receptor'].data['new_x']

        coors_ligand = batch_hetero_graph.nodes['ligand'].data['new_x']
        coors_receptor = batch_hetero_graph.nodes['receptor'].data['new_x']
        ...
        h_feats_ligand = self.residue_emb_layer(
            torch.argmax(batch_hetero_graph.nodes['ligand'].data['res_feat'], dim=1).unsqueeze(1).view(-1).long())
        h_feats_receptor = self.residue_emb_layer(
            torch.argmax(batch_hetero_graph.nodes['receptor'].data['res_feat'], dim=1).unsqueeze(1).view(-1).long())  # (N_res, emb_dim)
        ...
        all_ligand_T_align_list = []
        all_ligand_b_align_list = []
        all_receptor_T_align_list = []
        all_receptor_b_align_list = []
        ...
        for the_idx, hetero_graph in enumerate(list_hetero_graph):
            ...
            ## ligand
            # A
            A_lig = (Y_receptor_att_ROT - Y_receptor_att_ROT_mean).transpose(0,1) @ (Y_ligand_att_ROT - Y_ligand_att_ROT_mean) # 3, 3

            # SVD
            assert not torch.isnan(A_lig).any()
            U_lig, S_lig, Vt_lig = torch.linalg.svd(A_lig)

            num_it = 0
            while torch.min(S_lig) < 1e-3 or torch.min(torch.abs((S_lig**2).view(1,3) - (S_lig**2).view(3,1) + torch.eye(3).to(self.device))) < 1e-2:
                A_lig = A_lig + torch.rand(3,3).to(self.device) * torch.eye(3).to(self.device)
                U_lig, S_lig, Vt_lig = torch.linalg.svd(A_lig)
                num_it += 1
            ...
            corr_mat = torch.diag(torch.Tensor([1,1,torch.sign(torch.det(A_lig))])).to(self.device)
            T_align_ligand = (U_lig @  corr_mat) @ Vt_lig

            b_align_ligand = Y_receptor_att_ROT_mean - torch.t(T_align_ligand @ Y_ligand_att_ROT_mean.t())  # (1,3)
            
            ## receptor
            # A 
            A_rec = (Y_ligand_att_ROT - Y_ligand_att_ROT_mean).transpose(0,1) @ (Y_receptor_att_ROT - Y_receptor_att_ROT_mean) # 3, 3

            # SVD
            assert not torch.isnan(A_rec).any()
            U_rec, S_rec, Vt_rec = torch.linalg.svd(A_rec)

            num_it = 0
            while torch.min(S_rec) < 1e-3 or torch.min(torch.abs((S_rec**2).view(1,3) - (S_rec**2).view(3,1) + torch.eye(3).to(self.device))) < 1e-2:

                A_rec = A_rec + torch.rand(3,3).to(self.device) * torch.eye(3).to(self.device)
                U_rec, S_rec, Vt_rec = torch.linalg.svd(A_rec)
                num_it += 1
            ...
            corr_mat = torch.diag(torch.Tensor([1,1,torch.sign(torch.det(A_rec))])).to(self.device)
            T_align_receptor = (U_rec @  corr_mat) @ Vt_rec

            b_align_receptor = Y_ligand_att_ROT_mean - torch.t(T_align_receptor @ Y_receptor_att_ROT_mean.t())  # (1,3)

            #################### end AP 1 #########################
            all_ligand_T_align_list.append(T_align_ligand)
            all_ligand_b_align_list.append(b_align_ligand)
            all_receptor_T_align_list.append(T_align_receptor)
            all_receptor_b_align_list.append(b_align_receptor)

        return [all_ligand_T_align_list, all_ligand_b_align_list,
                all_receptor_T_align_list, all_receptor_b_align_list,
                all_Y_ligand_att_ROT_list, all_Y_receptor_att_ROT_list]
...
class Rigid_Body_Docking_Net(nn.Module):
    ...
    def __init__(self, args, log=None):
        ...
        if args['fine_tune']:
            ...
            self.list_iegmns = [('finetune', self.iegmn_original)] 
    ...
    def forward(self, batch_hetero_graph, epoch):
        ...
        all_receptor_coors_deform_list = []
        ...
        for stage, iegmn in self.list_iegmns:
            ...
            for the_idx, hetero_graph in enumerate(list_hetero_graph):
                ...
                orig_coors_receptor = hetero_graph.nodes['receptor'].data['new_x']
        
                # ligand 
                T_align_ligand = outputs[0][the_idx]  # rotation
                b_align_ligand = outputs[1][the_idx]  # translation
                # receptor
                T_align_receptor = outputs[2][the_idx]  # rotation
                b_align_receptor = outputs[3][the_idx]  # translation
                
                assert b_align_ligand.shape[0] == 1 and b_align_ligand.shape[1] == 3
                inner_coors_ligand = (T_align_ligand @ orig_coors_ligand.t() ).t() + b_align_ligand  # (n,3)
                assert b_align_receptor.shape[0] == 1 and b_align_receptor.shape[1] == 3
                inner_coors_receptor = (T_align_receptor @ orig_coors_receptor.t() ).t() + b_align_receptor  # (n,3)

                if stage == 'original':
                    hetero_graph.nodes['ligand'].data['new_x'] = inner_coors_ligand
                    hetero_graph.nodes['receptor'].data['new_x'] = inner_coors_receptor
                    new_list_hetero_graph.append(hetero_graph)

                if stage == 'finetune':
                    all_ligand_coors_deform_list.append(inner_coors_ligand)
                    all_receptor_coors_deform_list.append(inner_coors_receptor)

            if stage == 'original':
                batch_hetero_graph = dgl.batch(new_list_hetero_graph)
        
        all_keypts_ligand_list = last_outputs[4]
        all_keypts_receptor_list = last_outputs[5]
        all_ligand_rotation_list = last_outputs[0]
        all_ligand_translation_list = last_outputs[1]
        all_receptor_rotation_list = last_outputs[2]
        all_receptor_translation_list = last_outputs[3]

        return all_ligand_coors_deform_list,all_receptor_coors_deform_list, \
               all_keypts_ligand_list, all_keypts_receptor_list, \
               all_ligand_rotation_list, all_ligand_translation_list, all_receptor_rotation_list, all_receptor_translation_list
