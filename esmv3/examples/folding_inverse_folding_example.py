...
from esm3.sdk.api import (
    ESM3InferenceClient,
    ESMProtein,
    GenerationConfig,
    InverseFoldingConfig,
)
from esm3.sdk.forge import SequenceStructureForgeInferenceClient
...
def main(
    ...
    esm3._client: ESM3InferenceClient,
):
    ...
    esm3._client_folded_protein = esm3._client.generate(protein, config)
    assert isinstance(
        esm3._client_folded_protein, ESMProtein
    ...
    esm3._client_inv_folded_protein = cast(
        ESMProtein, esm3._client.generate(protein, config)
    )
    assert isinstance(
        esm3._client_inv_folded_protein, ESMProtein
    ), f"Using ESM3 client, ESMProtein was expected but got {protein}"
